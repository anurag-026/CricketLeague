...
export function ArenaBottomNav() {
  return (
    <nav
      aria-label="Arena navigation"
      className="safe-bottom fixed inset-x-0 bottom-0 z-50 flex h-20 items-center justify-around rounded-t-xl border-t border-primary-container/20 bg-surface-container/60 px-gutter shadow-[0_-4px_20px_rgba(0,218,243,0.15)] backdrop-blur-2xl md:hidden"
    >
      {navItems.map(({ to, label, icon, enabled }) =>
        enabled ? (
          ...
          >
            ...
            <span className="mt-1 font-mono text-xs font-medium tracking-widest uppercase">
              {label}
            </span>
          </NavLink>
        ) : (
          <button
            ...
            type="button"
            disabled
            className="flex flex-col items-center justify-center text-outline opacity-70"
